/// obd-ii trouble codes - conversion from integer value to description string


pub fn msg(trcode: isize) -> String {
    let tc = format!("P{:04}", trcode);
    for s in TROUBLECODES {
        if s.as_bytes().starts_with(tc.as_bytes()) {
...
const TROUBLECODES: &'static [ &'static str ] = &[
  ...
  "P0618 Alternative Fuel Control Module KAM (Keep Alive Memory) Error",
  ...
  "P2096 Post Catalyst Fuel Trim System Too Lean Ban k1",
